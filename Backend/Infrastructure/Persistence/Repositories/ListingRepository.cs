    ...
    public class ListingRepository : IListingRepository {
		...
		private async Task<Listing?> ValidateAndAddPropertyValue(Listing listing, List<IListingPropertyValueSelection> valueSelection, Category category) {
			...
			foreach (var property in category.ListingProperties) {
				var selection = valueSelection.Find(selection => selection.ListingPropertyId == property.Id);
				if (selection == null) {
		...
		public async Task<Listing?> DeleteAsync(Guid id) {
			...
			foreach(Image image in existingListing.Images) {
				imageRepository.DeleteImageFromFileSystem(image);
			}
		...
		public async Task<Page<Listing>> GetAllAsync(ListingFilter filter, int pageNumber = 1, int pageSize = 20) {
            var city = await dbContext.Cities.Where(x => x.Id == filter.CityId).FirstOrDefaultAsync();
			var q = dbContext.Listings
				.Include(l => l.Images)
				.Include(l => l.SelectedListingPropertyValues)
				.Include(l => l.Category)
				.ApplyAllListingsFilters(filter, city)
		...
		public async Task<Listing?> UpdateAsync(Guid id, Listing updatedListing, List<IListingPropertyValueSelection>? valueSelections = null) {
			var existing = await dbContext.Listings
				.Include(x => x.SelectedListingPropertyValues)
				.FirstOrDefaultAsync(x => x.Id == id);
			if (existing == null) {
			...
			if (valueSelections != null) {
				var category = await dbContext.Categories
					.Include(c => c.ListingProperties)
					.FirstOrDefaultAsync(c => c.Id == updatedListing.CategoryId);
				if (category == null) {
					return null;
				}
				var validatedListing = await ValidateAndAddPropertyValue(updatedListing, valueSelections, category);
				if (validatedListing == null) {
					return null;
				}
				existing.SelectedListingPropertyValues = validatedListing.SelectedListingPropertyValues;
			}

			existing.IsSold = updatedListing.IsSold;
			existing.IsArchived = updatedListing.IsArchived;
			...
			existing.CategoryId = updatedListing.CategoryId;
			existing.CityId = updatedListing.CityId;
			await dbContext.SaveChangesAsync();
			return existing;
		}
